...
public interface PayableAmountMapper {
    ...
    List<PayableAmount> selectByYearWithPage(@Param("year") Integer year, @Param("status") String status, @Param("keyword") String keyword, @Param("offset") int offset, @Param("limit") int limit);

    int countByYearWithFilter(@Param("year") Integer year, @Param("status") String status, @Param("keyword") String keyword);
}
